...
const DataInput = ({ label, idHtmlFor, type = "text", name, warningText }) => {
  ...
  return (
    <>
      <div className='flex flex-col text-lg space-y-1 sm:text-base'>
        <label htmlFor={idHtmlFor}>{label}</label>
        <input onInput={showWarningHandler} ref={inputRef} type={type} id={idHtmlFor} name={name} className={`outline-none border rounded-lg p-3 ${isWarningVisible ? "border-red-500" : "border-gray-400"}`} />
        {isWarningVisible &&
